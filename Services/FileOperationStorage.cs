...
namespace Task1.Services
{
    public class FileOperationStorage : IFileOperation
    {
        ...
        private readonly static string connectionString = "Server=localhost;User ID=root;Password=;Database=task1;Connection Timeout=60";
        ...
        public int BATCH_SIZE { get; set; } = 100;
        ...
        public async Task<int> SaveToDb(string query)
        {
            try {
               await IFileOperation.SaveToDb(connectionString, query);
            } catch(Exception e) {
                Console.WriteLine($"Exception in FOS: {e.Message}");
                Thread.Sleep(2000);
                return 0;
            }
            return 1;
